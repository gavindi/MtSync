...
## 0.9.20 — Dynamic (Watch-Triggered) Sync Mode

- **Watch a job's local source directory for changes and sync automatically**: Sync/Copy/Move jobs (including bisync) can now enable a new "Watch for Changes" toggle (Schedule tab) that triggers a run shortly after files change under a local `source`, instead of relying solely on a cron schedule or manual "Run Now". Implemented via a new recursive `Gio::FileMonitor`-based watcher (`src/watch/directory_watcher.hpp`/`.cpp`, wraps inotify on Linux) that maintains one monitor per directory — since neither `Gio::FileMonitor` nor raw inotify watch subtrees — dynamically adding and removing monitors as subdirectories are created and deleted, with a symlink-loop guard and a 32-level recursion depth cap as a safety valve against exhausting `fs.inotify.max_user_watches` on very large or deeply nested trees
- **Debounced, storm-resistant triggering**: a burst of filesystem events (e.g. a bulk delete or directory move firing thousands of inotify notifications) is coalesced into a single sync run via a debounce-with-max-wait scheduler in the daemon (`MtSyncDaemon::on_watch_changed`/`fire_watch_triggered_run`) — each new event resets a short "quiet period" timer (default 2s, `Job::watch_debounce_ms` / `Settings::watch_debounce_ms`, `-1` meaning "use global default", the same convention already used by `parallel_transfers`/`retries`), but a hard cap (default 30s, `watch_max_wait_ms`) guarantees a sync still fires periodically even under continuous churn rather than being postponed indefinitely. Watch-triggered runs go through `on_run_job()`'s existing in-flight guard unchanged (no double-submission if a previous run is still going) and re-check for a pending trigger once that run completes, so a change is never silently dropped
- **Watch lifecycle follows job config exactly like cron scheduling**: watches are (re)built from `jobs.json` in a new `setup_all_watches()`, called at daemon startup and after every `add_job`/`update_job`/`delete_job`, mirroring the existing `schedule_all_jobs()`. Watches are pure in-memory state, never persisted, and are fully torn down and rebuilt on every daemon restart
- **New job option, gated to what it can actually watch**: "Watch for Changes" is hidden for Mount jobs (no sync semantics) and greyed out with an explanatory tooltip when `source` is an rclone remote (`remote:path`) rather than a local directory — a new `watch::is_local_path()` helper distinguishes the two using rclone's remote-name grammar. Bisync jobs can still enable it to react quickly to local-side edits; remote-side changes still require the existing cron schedule, since inotify can't observe them
- **New global defaults**: "Watch mode debounce (ms)" and "Watch mode max wait (ms)" rows added to Settings → Transfers, alongside Parallel Transfers/Retries
- **Activity log now distinguishes watch-triggered runs**: `STARTED` lines append `(triggered by file watch)` when a run was fired by the watcher rather than a schedule or manual "Run Now"
- **Fixed: daemon crashed the first time anything used a giomm-wrapped `GObject`**: the `--daemon` process runs headless (no `Gtk::Application`), so `Gio::init()` was never called and giomm's type-wrapper table was never populated; introducing `Gio::FileMonitor` for watch mode was the first code path to exercise this gap, crashing on startup with `Failed to wrap object of type 'GInotifyFileMonitor'`. `main.cpp` now calls `Gio::init()` before constructing `MtSyncDaemon` when running as `--daemon`
...
- **Browse tab error description silently blanked with a GTK-WARNING**: `BrowserPane::navigate()`'s error page passed rclone's raw stderr straight into `AdwStatusPage`'s description, which is rendered as Pango markup — rclone's syslog-style `<N>ERROR`/`<5>NOTICE` line prefixes are not valid markup, so the description failed to render (`Failed to set text ... from markup due to error parsing markup`) and the real error text never reached the user. Error text is now escaped with `Glib::Markup::escape_text()` before being set
- **Local filesystem errors mislabeled "Unable to Connect"**: the Browse tab's error page used the same "Unable to Connect" title and network icon for every `lsjson` failure, which is misleading for a purely local filesystem error (no connection is involved). Local (`Local` source) failures now show "Unable to Read Directory" with a generic error icon; remote failures keep "Unable to Connect" with the network icon

## 0.9.15 — First-time Welcome Tour & Tray Animation Use-After-Free Fix

- **First-time welcome tour**: on first launch, a new `WelcomeDialog` greets the user with a "Skip"/"Take the Tour" status page (plus a pre-checked "Don't show this again") that transitions into a 6-page `AdwCarousel` walkthrough with dot indicators and Back/Next navigation, covering: adding a remote, choosing source/destination, picking a job type, setting job options, running it, and optional scheduling. Controlled by a new `show_welcome_on_startup` setting (default `true`, persisted to `settings.json`); the About tab gained a "Quick Start Tour" button to replay it anytime ([#10](https://github.com/gavindi/MtSync/issues/10))
...
- **Duplicate flatpak CI artifact**: `flatpak/flatpak-github-actions/flatpak-builder@v6`'s `upload-artifact` input defaults to `true`, so it auto-uploaded the built bundle as its own artifact in addition to the workflow's own explicit `actions/upload-artifact@v4` step — every flatpak build produced two artifacts containing the same file. Set `upload-artifact: false` on the flatpak-builder step in `.github/workflows/build.yaml` and `.github/workflows/release.yml` so only the explicit step uploads it
...
- **Spurious job resubmission during checksum-heavy bisync**: a single missed `job/status` poll was treated as fatal job failure, causing the daemon to resubmit a brand-new bisync job while the original was often still running — two concurrent bisync runs against the same pair then collided on its workdir lock (`hash: failed to read: context canceled`), consuming hundreds of rclone job IDs without ever completing; polling now tolerates up to 6 consecutive failed polls (tracked via a new `JobState::poll_failures` counter) before giving up, and the shared `SoupSession` HTTP timeout is raised from 5s to 20s to reduce spurious timeouts while `rcd` is busy hashing files
- **First-run bisync baseline**: bisync refuses to run without an established baseline, aborting with `bisync aborted` on a pair that had never been synced before. Proactively forcing `resync: true` until a run succeeded (the initial fix) turned out to be unsafe — since bisync's own `--resync` re-establishes the baseline from current state on *every* invocation, forcing it repeatedly after unrelated failures could resurrect files the user had just deleted, and could misfire the delete-safety check. Replaced with a reactive, single-shot retry: a normal run is always attempted first; only when rclone's captured log reports it can't find a prior baseline (`cannot find prior Path1 or Path2 listings`) does the daemon retry once with `resync: true` (`JobState::bisync_force_resync` / `bisync_resync_retried`). Other bisync aborts (e.g. empty-directory, too-many-deletes) are left alone rather than auto-"fixed"
- **`JobStatus` now captures rclone's raw job log**: `job/status` only ever surfaced the generic top-level `error` string, which for bisync is the same `"bisync aborted"` for several unrelated causes (missing baseline, empty directory, too many deletes) — indistinguishable without more detail. `JobStatus` gained an `output_log` field populated from the RC response's `output.output` text, used to tell these cases apart reliably
- **`maxDelete` ignored by rclone's RC handler**: bisync's delete-safety check aborted on *any* deletion, even a single file — rclone's `sync/bisync` RC endpoint disregards the `maxDelete` parameter entirely (verified with values 50 and 100: no effect) and behaves as if the threshold were 0%, unlike the CLI's documented 50% default. Since there's no way to get a real percentage threshold over RC, added a per-job opt-in **Force Deletes** switch (bisync jobs only, default off, `Job::bisync_force_deletes`) that sends `force: true` to bypass the check — off by default so a single deletion still blocks the sync for review, on to let deletions of any size propagate with no protection against a large accidental deletion
